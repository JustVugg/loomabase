...
export type ReplicaPredicate =
  | { kind: "id_equals"; value: string }
  | { kind: "id_prefix"; value: string }
  | { kind: "column_equals"; value: { column: string; value: CrdtValue } };
...
export const TODOS_TABLE: TableContract = Object.freeze({
  name: "todos",
  columns: Object.freeze([
    Object.freeze({ name: "title", type: "text" as const }),
    Object.freeze({ name: "completed", type: "boolean" as const }),
  ]),
});
...
export function text(value: string): CrdtValue {
  return { type: "text", value };
}

export function booleanValue(value: boolean): CrdtValue {
  return { type: "boolean", value };
}

export function integer(value: number): CrdtValue {
  assertSafeInteger("integer value", value);
  return { type: "integer", value };
}

export function real(value: number): CrdtValue {
...
export function nullValue(): CrdtValue {
  return { type: "null" };
}

export function blob(value: Uint8Array | number[]): CrdtValue {
  return { type: "blob", value: Array.from(value) };
}

export function fingerprintContract(contract: TableContract): bigint {
...
export function emptyPayload(
  ...
  sourceLamport: bigint | number = 0n,
  table: TableContract = TODOS_TABLE,
): SyncPayload {
...
export function compareMetadata(current: ColumnMetadata, incoming: ColumnMetadata): number {
  if (incoming.lamport_clock > current.lamport_clock) return 1;
  if (incoming.lamport_clock < current.lamport_clock) return -1;
  if (incoming.device_id > current.device_id) return 1;
  if (incoming.device_id < current.device_id) return -1;
  return 0;
}

export function decideLww(current: ColumnMetadata, incoming: ColumnMetadata): MergeDecision {
  const comparison = compareMetadata(current, incoming);
  if (comparison > 0) return "accept_incoming";
  if (comparison < 0) return "keep_current";
  return "equal";
}

export function explainLww(
  current: CrdtColumn | undefined,
  incoming: CrdtColumn,
): ConflictExplanation {
  if (!current) {
    return {
  ...
  if (decision === "accept_incoming") {
    ...
    return {
      ...
      current: current.metadata,
      incoming: incoming.metadata,
      summary: `clocks are equal and incoming device ID ${JSON.stringify(
        incoming.metadata.device_id,
      )} sorts after current device ID ${JSON.stringify(current.metadata.device_id)}`,
    };
  }
  if (decision === "keep_current") {
    ...
    return {
      ...
      summary: `clocks are equal and incoming device ID ${JSON.stringify(
        incoming.metadata.device_id,
      )} sorts before current device ID ${JSON.stringify(current.metadata.device_id)}`,
    };
  }
...
export function stringifySyncPayload(value: unknown): string {
  const encoded = JSON.stringify(value, (_key, fieldValue: unknown) => {
  ...
  return encoded.replace(
    new RegExp(`"${BIGINT_MARKER_PREFIX}(-?\\d+)${BIGINT_MARKER_SUFFIX}"`, "g"),
    "$1",
  );
}

export function parseSyncPayload(json: string): SyncPayload {
  return parseWireJson(json) as SyncPayload;
}

export function parsePartialReplicaResponse(json: string): PartialReplicaResponse {
  return parseWireJson(json) as PartialReplicaResponse;
}

export function parseWireJson(json: string): unknown {
  const rewritten = json.replace(
    /"(schema_fingerprint|source_lamport|lamport_clock|lamport|cursor|scope_version)"\s*:\s*(-?\d+)/g,
    '"$1":"$2"',
  );
  return JSON.parse(rewritten, (key: string, value: unknown) => {
...
export function stringifyReplicaSnapshot(snapshot: TodoReplicaSnapshot): string {
  return stringifySyncPayload(snapshot);
}

export function parseReplicaSnapshot(json: string): TodoReplicaSnapshot {
  const snapshot = parseWireJson(json) as TodoReplicaSnapshot;
...
export class LoomabaseHttpClient {
  ...
  private readonly getToken: LoomabaseHttpClientOptions["getToken"] | undefined;
  ...
  async sync(payload: SyncPayload): Promise<SyncPayload> {
    const response = await this.postJson(this.endpoint, payload);
    return parseSyncPayload(response);
  }

  async syncPartial(request: PartialReplicaRequest): Promise<PartialReplicaResponse> {
  ...
  private async postJson(url: URL, body: unknown): Promise<string> {
...
export class ReferenceSyncServer {
  ...
  globalLamport = 0n;
  cells = new Map<string, CrdtColumn>();
  private seq = 0n;
  private cellSeq = new Map<string, bigint>();

  constructor(options: { table?: TableContract } = {}) {
    this.table = options.table ?? TODOS_TABLE;
  }

  merge(payload: SyncPayload, authenticatedDeviceId = payload.source_device_id): SyncPayload {
    validateClientPayload(payload, authenticatedDeviceId, this.table);
    const observed = maxObservedClock(payload);
    const responseRows = new Map<string, Record<string, CrdtColumn>>();
...
export class LocalStorageTodoReplicaStorage implements TodoReplicaStorage {
  ...
  async load(): Promise<TodoReplicaSnapshot | null> {
    const value = this.storage.getItem(this.key);
    return value ? parseReplicaSnapshot(value) : null;
  }

  async save(snapshot: TodoReplicaSnapshot): Promise<void> {
...
export class JsonFileTodoReplicaStorage implements TodoReplicaStorage {
...
export class MemoryTodoReplica {
  ...
  private cells = new Map<string, CrdtColumn>();
  private dirty = new Set<string>();

  constructor(options: { deviceId: string; table?: TableContract }) {
  ...
  static fromSnapshot(
    ...
    options: { deviceId?: string; table?: TableContract } = {},
  ): MemoryTodoReplica {
    validateSnapshot(snapshot);
  ...
  async save(storage: TodoReplicaStorage): Promise<void> {
    await storage.save(this.snapshot());
  }

  createTodo(id: string, titleValue: string, completed = false): void {
  ...
  localDelta(): SyncPayload {
    const rows = new Map<string, Record<string, CrdtColumn>>();
  ...
  acknowledge(sent: SyncPayload, response?: SyncPayload): void {
    ...
    const rejected = new Set(
      (response?.rejections ?? []).map((rejection) =>
        versionKey(
          rejection.todo_id,
          rejection.column_name,
  ...
  async syncWith(transport: (payload: SyncPayload) => Promise<SyncPayload>): Promise<SyncPayload> {
  ...
  private writeCell(id: string, columnName: string, value: CrdtValue): void {
  ...
  private materialize(id: string): void {
    ...
    this.todos.set(id, {
      ...
      completed:
        completedValue?.type === "boolean" ? completedValue.value : existing.completed,
...
function validateClientPayload(
  ...
  if (payload.source_device_id !== authenticatedDeviceId) {
    throw new LoomabaseClientError(
      "source_device_id does not match the authenticated device",
    );
  }
  for (const row of payload.changes) {
    for (const column of Object.values(row.columns)) {
      if (column.metadata.device_id !== authenticatedDeviceId) {
        throw new LoomabaseClientError(
          "a client cannot attribute a change to another device",
        );
      }
    }
  }
...
function validateServerPayload(payload: SyncPayload, table: TableContract): void {
  validatePayload(payload, table);
...
function validatePayload(payload: SyncPayload, table: TableContract): void {
...
function validateSnapshot(snapshot: TodoReplicaSnapshot): void {
...
function validateColumn(table: TableContract, columnName: string, value: CrdtValue): void {
...
function synchronizedColumns(table: TableContract): ColumnDef[] {
...
function maxObservedClock(payload: SyncPayload): bigint {
...
function sameCrdtValue(left: CrdtValue, right: CrdtValue): boolean {
  return JSON.stringify(left) === JSON.stringify(right);
}

function cloneColumn(column: CrdtColumn): CrdtColumn {
  return {
...
function cloneTable(table: TableContract): TableContract {
  return {
    name: table.name,
...
function cellKey(todoId: string, columnName: string): string {
  return `${encodeURIComponent(todoId)}|${encodeURIComponent(columnName)}`;
}

function splitCellKey(key: string): [string, string] {
  const separator = key.indexOf("|");
  if (separator === -1) throw new LoomabaseClientError("invalid cell key");
  return [
